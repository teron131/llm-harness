...
export type EvalModelsUnionPayload = {
  aa_fetched_at_epoch_seconds: number;
  aa_status_code: number;
  models_dev_fetched_at_epoch_seconds: number;
  models_dev_status_code: number;
  total_aa_models: number;
...
function numericMatchBonus(aaSlug: string, modelId: string): number {
  const aaTokens = splitTokens(aaSlug);
  const modelTokens = splitTokens(splitBaseModelId(modelId));
  const maxLength = Math.min(aaTokens.length, modelTokens.length);
  for (let index = 0; index < maxLength; index += 1) {
    const aaToken = aaTokens[index];
    const modelToken = modelTokens[index];
    if (isNumericToken(aaToken) && isNumericToken(modelToken)) {
      return aaToken === modelToken ? NUMERIC_EXACT_MATCH_REWARD : 0;
    }
  }
  return 0;
}

function numericClosenessBonus(aaSlug: string, modelId: string): number {
  const aaNumbers = splitTokens(aaSlug)
  ...
  const maxLength = Math.max(aaNumbers.length, modelNumbers.length);
  for (let index = 0; index < maxLength; index += 1) {
    const aaValue = aaNumbers[index];
    const modelValue = modelNumbers[index];
    if (aaValue == null || modelValue == null) {
      return 0;
    }
    if (aaValue === modelValue) {
      continue;
    }
    return NUMERIC_CLOSENESS_SCALE / (1 + Math.abs(aaValue - modelValue));
  }
...
function sameVariantBonus(
  ...
  const aaLastToken = aaTokens[aaTokens.length - 1];
  if (!aaLastToken || isNumericToken(aaLastToken)) {
    return 0;
  }
  const baseLastToken = modelBaseTokens[modelBaseTokens.length - 1];
  const nameLastToken = modelNameTokens[modelNameTokens.length - 1];
  if (aaLastToken === baseLastToken || aaLastToken === nameLastToken) {
...
function setCoverageBonus(
  ...
  const aaSet = new Set(splitTokens(aaSlug));
...
function hasFirstTokenMatch(
  ...
  const aaFirst = splitTokens(aaSlug)[0];
  if (!aaFirst) {
    return false;
  }
  return (
    aaFirst === splitTokens(splitBaseModelId(modelId))[0] ||
    aaFirst === splitTokens(modelName)[0]
  );
}
...
function scoreCandidate(
  ...
  const normalizedAa = normalize(aaSlug);
  const normalizedModelBase = normalize(splitBaseModelId(modelId));
  const normalizedModelName = normalize(modelName);
  const aaTokens = splitTokens(aaSlug);
  const modelBaseTokens = splitTokens(splitBaseModelId(modelId));
  const modelNameTokens = splitTokens(modelName);
  const prefixBase = commonPrefixLength(normalizedAa, normalizedModelBase);
  const prefixName = commonPrefixLength(normalizedAa, normalizedModelName);
  ...
  const weightedTokenScore = Math.max(
    weightedTokenPrefixScore(aaTokens, modelBaseTokens),
    weightedTokenPrefixScore(aaTokens, modelNameTokens),
  );

  return (
    weightedTokenScore * TOKEN_PREFIX_MULTIPLIER +
    numericMatchBonus(aaSlug, modelId) +
    numericClosenessBonus(aaSlug, modelId) +
    sameVariantBonus(aaSlug, modelId, modelName) +
    setCoverageBonus(aaSlug, modelId, modelName) +
    maxPrefix * CHAR_PREFIX_REWARD_SCALE -
    Math.abs(normalizedAa.length - normalizedModelBase.length) *
...
export async function getEvalModelsUnion(
  ...
  const rows: EvalModelUnionRow[] = evalStats.models.map((evalModel) => {
    ...
    return {
      aa_slug: typeof evalModel.slug === "string" ? evalModel.slug : "",
      aa_name: typeof evalModel.name === "string" ? evalModel.name : null,
      aa_release_date:
  ...
  return {
    aa_fetched_at_epoch_seconds: evalStats.fetched_at_epoch_seconds,
    aa_status_code: evalStats.status_code,
    models_dev_fetched_at_epoch_seconds: modelStats.fetched_at_epoch_seconds,
    models_dev_status_code: modelStats.status_code,
    total_aa_models: evalStats.models.length,
