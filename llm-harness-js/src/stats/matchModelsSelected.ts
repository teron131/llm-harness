...
import { getMatchModelsUnion } from "./matchModels.js";
...
export type MatchSelectedModel = {
  id: string | null;
  name: string | null;
...
export type MatchModelsSelectedPayload = {
  models: MatchSelectedModel[];
};
...
export async function getMatchModelsSelected(): Promise<MatchModelsSelectedPayload> {
  const union = await getMatchModelsUnion();
  const models = union.models.map((row): MatchSelectedModel => {
  ...
  const payload: MatchModelsSelectedPayload = { models };
