...
- `llm_harness/agents/youtube/summarizer.py -> create_summarizer_agent/summarize_video`
...
- `summarizer_react.py` is the strongest control loop:
  garbage filter -> summary -> quality check -> conditional refinement until acceptable or max iterations.
- `summarizer.py` pushes transcript filtering into tool middleware before final structured summary generation.
...
- Choose `summarizer_react.py` when you need iterative quality enforcement.
- Choose `summarizer.py` for simpler agentic flows with middleware-based cleaning.
